...
describe('Array', function() {
  ...
  describe('change/splice events', function() {
    var a, spy;
    ...
    it('emits a `change:<prop name>` event when a property on an element of the array changes', function() {
      a.at(0).x = 10;
    ...
    it('emits a `splice:<prop name>` event when a property emits a `splice` event', function() {
      a.at(0).emit('splice:x');
      expect(spy).toHaveBeenCalledWith('splice:x', {array: a});
    });

    it('emits a `change:<prop name>` event when an added element changes', function() {
    ...
    it('emits a `splice:<prop name>` event when an added element emits a `splice` event', function() {
      var t = new Test({x: 4, y: 'd'});
      a.push(t);
      t.emit('splice:y');
      expect(spy).toHaveBeenCalledWith('splice:y', {array: a});
    });

    it('does not emit a `change:<prop name>` event when a removed element changes', function() {
      var t = a.pop();
      t.y = 'e';
      expect(spy).not.toHaveBeenCalled();
    });

    it('does not emit a `splice:<prop name>` event when a removed element emits a `splice` event', function() {
      var t = a.pop();
      t.emit('splice:x');
      expect(spy).not.toHaveBeenCalled();
    });
